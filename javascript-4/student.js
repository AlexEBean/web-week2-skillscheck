...
// CODE HERE
const faveColors = ["green", "red", "black"]
...
const me = {
    firstName: "Alex",
    superHeroName: "Bogus Nocturnal Architect",
    homeTown: "Highland",
    superPowers: ["Empathic Mimicry", "psychic generation", "stopping time"],
...
function setColor(arr) {
    let threeColors = faveColors.slice(0,3)
    for (let i = 0; i < threeColors.length; i++){
        if (threeColors[i] === "blue"){
            threeColors[i] = "#4D4DFF"
        }
    }
    background(threeColors[0], threeColors[1], threeColors[2])
}
